  ...
  (
    ...
    React.useEffect(() => {
      ...
      if (isPopoverOpen !== prevIsOpen.current) {
        if (isPopoverOpen) {
          announce(`Menu suspenso aberto. ${totalOptions} opções disponíveis. Use as teclas de seta para navegar.`);
        } else {
          announce("Menu suspenso fechado.");
        }
      ...
      <>
        ...
        <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen} modal={modalPopover}>
          <div id={triggerDescriptionId} className="sr-only">
            Multi-seleção. Use as teclas de seta para navegar, Enter para selecionar, e Escape para fechar.
          </div>
          <div id={selectedCountId} className="sr-only" aria-live="polite">
            {selectedValues.length === 0
              ? "Nenhuma opção selecionada"
              : `${selectedValues.length} opção${selectedValues.length === 1 ? "" : "s"} selecionada${selectedValues.length === 1 ? "" : "s"}: ${selectedValues
                ...
                <div className="flex w-full items-center justify-between">
                  ...
                  >
                    ...
                    {selectedValues.length > responsiveSettings.maxCount && (
                      ...
                      >
                        {`+ ${selectedValues.length - responsiveSettings.maxCount} mais`}
                        <XCircle
                          className={cn(
            ...
            <Command>
              {searchable && (
                <CommandInput
                  ...
                  aria-label="Buscar nas opções disponíveis"
                  aria-describedby={`${multiSelectId}-search-help`}
                />
              )}
              {searchable && (
                <div id={`${multiSelectId}-search-help`} className="sr-only">
                  Digite para filtrar opções. Use as teclas de seta para navegar nos resultados.
                </div>
              )}
              <CommandList
                className={cn(
              ...
              >
                <CommandEmpty>{emptyIndicator || "Nenhum resultado encontrado."}</CommandEmpty>{" "}
                {!hideSelectAll && !searchValue && (
                  <CommandGroup>
                    <CommandItem
                      ...
                      aria-label={`Selecionar todas as ${getAllOptions().length} opções`}
